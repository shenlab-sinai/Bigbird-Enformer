...
class BigBirdAttention(nn.Module):
    ...
    def __init__(self, d_model, num_heads, block_size=64, dim_key=64, dim_value=64):
        super().__init__()
        ...
        self.inner_k = num_heads * dim_key      # 8*64 = 512
        self.inner_v = num_heads * dim_value    # 8*64 = 512

        # projections into attention space (512), not d_model (1536)
        ...
        # back to model dim
        self.out_proj = nn.Linear(self.inner_v, d_model, bias=True)
    ...
    def forward(self, x, global_indices=None):
        ...
        # q,k: [B, H, N, dim_key]
        q = self.q_proj(x).view(batch_size, seq_len, self.num_heads, self.dim_key).transpose(1, 2)
        k = self.k_proj(x).view(batch_size, seq_len, self.num_heads, self.dim_key).transpose(1, 2)

        # v: [B, H, N, dim_value]
        v = self.v_proj(x).view(batch_size, seq_len, self.num_heads, self.dim_value).transpose(1, 2)

        context = bigbird_block_sparse_attention(q, k, v, self.block_size, global_indices)

        # [B, N, H*dim_value] -> [B, N, d_model]
        context = context.transpose(1, 2).reshape(batch_size, seq_len, self.inner_v)
