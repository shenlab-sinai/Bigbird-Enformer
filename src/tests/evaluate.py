...
CKPT_PATH = os.path.join(
    project_root,
    "tb_logs", "bigbird_enformer", "new_version", "checkpoints",
    "bigbird-best-epoch=133-val_corr_coef=0.6738.ckpt",   # update filename as needed
)
...
# Track indices from official Enformer metadata
# Source: https://github.com/deepmind/enformer (targets_human.txt)
PLOT_TRACKS = {
    "DNASE:CD14 (ENCODE)":      45,    # DNase
    "H3K27ac:HepG2 (ENCODE)":  694,    # Histone ChIP
    "CAGE:liver":              4828,    # CAGE
}

# Assay slices for breakdown table (Enformer paper convention)
ASSAY_SLICES = {
    "DNASE": slice(0, 674),
    "CHIP":  slice(674, 4675),
...
class TestDataModule(pl.LightningDataModule):
    def __init__(self, npz_dir, batch_size=4):
        super().__init__()
        self.npz_dir = npz_dir
        self.batch_size = batch_size

    def setup(self, stage=None):
        self.test_ds = SingleOrganismDataset(self.npz_dir, split="test")
        print(f"Test set: {len(self.test_ds)} sequences")
...
class PearsonCorrPerChannel(Metric):
    """Streaming Pearson R per output channel, matching training metric."""
...
class EvalWrapper(pl.LightningModule):
    ...
    @torch.no_grad()
    def _predict(self, seq: torch.Tensor) -> torch.Tensor:
        if self.use_official:
            out = self.model(seq)
            return out["human"] if isinstance(out, dict) else out
        else:
            out = self.model.model(seq)
            return out["human"]
    ...
    def test_step(self, batch, batch_idx):
        ...
        pred = self._predict(seq)    # [B, 896, 5313]
        self.corr_metric.update(pred, target)

        # Plot one batch worth of tracks
    ...
    def on_test_epoch_end(self):
        ...
        # Per-channel distribution plot
        self._plot_corr_distribution(corr)

    def _plot(self, pred: torch.Tensor, target: torch.Tensor):
    ...
    def _plot_corr_distribution(self, corr: np.ndarray):
...
if __name__ == "__main__":
    if USE_OFFICIAL:
        ...
        assert os.path.exists(os.path.join(OFFICIAL_ID, "config.json")), "config.json not found"
        assert os.path.exists(os.path.join(OFFICIAL_ID, "pytorch_model.bin")), "pytorch_model.bin not found"
        ...
        config = EnformerConfig(
            ...
            block_size=128,
            use_checkpointing=False,   # not needed for eval
            attn_dropout=0.05,
            dropout_rate=0.4,
            pos_dropout=0.01,
        )
    ...
    dm = TestDataModule(HUMAN_NPZ_DIR, batch_size=BATCH_SIZE)
